...
`dev` selects 300 SKU-store series per state, keeps the production graph, uses three Optuna
trials, and targets a 15 to 30 minute feedback loop. `full` processes all 30,490 series, requires
the GPU, uses twelve tuning trials, and targets a one to four hour run on the reference machine.
