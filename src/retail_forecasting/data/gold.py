...
def add_origin_features(daily: DataFrame, config: ProjectConfig) -> DataFrame:
    ...
    for lag in config.features.lags:
        frame = frame.withColumn(f"lag_{lag}", F.lag("units", lag).over(history))
    for width in config.features.rolling_windows:
        trailing = history.rowsBetween(-width, -1)
        frame = (
    ...
    all_history = history.rowsBetween(Window.unboundedPreceding, -1)
...
def _training_origins(config: ProjectConfig) -> list[int]:
    ...
    first = max(max(config.features.lags), last - config.data.history_days)
